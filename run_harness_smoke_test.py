...
def run_smoke() -> int:
    ...
    # -----------------------------------------------------------------------
    # Section 1: Stub agents complete via Supervisor
    # -----------------------------------------------------------------------
    ...
    # -----------------------------------------------------------------------
    # Section 3: Metadata tool allowed under permissive SafetyConfig
    # -----------------------------------------------------------------------
    ...
    # Use a known file inside HARNESS_ROOT; no content is returned, only metadata.
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if result_m.task_outputs:
            ...
            check(results, "extension present",          "extension" in meta)
            check(results, "last_modified present",      "last_modified" in meta)
    ...
    # -----------------------------------------------------------------------
    # Section 4: Metadata tool blocked by default SafetyConfig
    # -----------------------------------------------------------------------
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        result_b = Supervisor(state_store=store_b).process(job_b)  # default: deterministic_only=True
    ...
    # -----------------------------------------------------------------------
    # Section 5: Path safety checks
    # -----------------------------------------------------------------------
    run_section("Path safety checks")

    check(results, "path traversal rejected",
          inspect_file_metadata("../../etc/passwd")["status"] == "denied")
    check(results, "traversal reason text present",
          "traversal" in inspect_file_metadata("../../etc/passwd").get("reason", "").lower())
    check(results, "absolute escape rejected",
          inspect_file_metadata("/etc/passwd")["status"] == "denied")
    check(results, ".env rejected",      inspect_file_metadata(".env")["status"] == "denied")
    check(results, ".pem rejected",      inspect_file_metadata("cert.pem")["status"] == "denied")
    check(results, ".key rejected",      inspect_file_metadata("id_rsa.key")["status"] == "denied")
    check(results, "directory rejected", inspect_file_metadata("harness")["status"] == "denied")

    # -----------------------------------------------------------------------
    # Section 6: No content/preview returned
    # -----------------------------------------------------------------------
    run_section("No content returned")

    known = inspect_file_metadata("harness/__init__.py")
    if known.get("exists"):
        forbidden = {"content", "preview", "snippet", "first_bytes", "text", "data"}
        check(results, "no content key in result", not (set(known.keys()) & forbidden))
        check(results, "line_count is integer",
              isinstance(known.get("line_count", 0), int))
    else:
        check(results, "known file found", False, str(known))

    # -----------------------------------------------------------------------
    # Section 7: No file mutation
    # -----------------------------------------------------------------------
    run_section("No mutation")

    with tempfile.TemporaryDirectory() as tmpdir:
        canary = Path(tmpdir) / "canary.txt"
        canary.write_text("original", encoding="utf-8")
        check(results, "out-of-root file rejected",
              inspect_file_metadata(str(canary))["status"] == "denied")
        check(results, "canary file unchanged", canary.read_text() == "original")

    # -----------------------------------------------------------------------
    # Section 8: Drift diagnostic
    # -----------------------------------------------------------------------
    run_section("Drift diagnostic (orchestrator_v2.py)")

    proc = subprocess.run(
        ["/home/jfroh/hermes/harness_venv/bin/python",
         "/home/jfroh/hermes/harness/orchestrator_v2.py"],
        capture_output=True, text=True, timeout=15,
    )
    check(results, "orchestrator_v2 exits 0", proc.returncode == 0,
          f"rc={proc.returncode} stderr={proc.stderr[:120]}" if proc.returncode != 0 else "")
    if proc.returncode == 0:
        try:
            parsed = json.loads(proc.stdout)
            check(results, "orchestrator_v2 output is JSON dict", isinstance(parsed, dict))
        except Exception as exc:
            check(results, "orchestrator_v2 output is JSON dict", False, str(exc))
